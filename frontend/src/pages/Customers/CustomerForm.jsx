...
const CustomerForm = () => {
    ...
    const getInitialState = () => {
        if (!isEditMode) {
            const saved = sessionStorage.getItem('customerFormDraft');
    ...
    const [customCategory, setCustomCategory] = useState(() => {
        if (!isEditMode) {
            const saved = sessionStorage.getItem('customerFormDraft_customCategory');
    ...
    const fetchCustomer = async () => {
        try {
            ...
            if (data.customer.financeStartDate) {
                try {
                    formattedDate = new Date(data.customer.financeStartDate).toISOString().split('T')[0];
                } catch (e) {}
            }
            
            setFormData({
                ...data.customer,
                financeStartDate: formattedDate
            });
        } catch (error) {
    ...
    useEffect(() => {
        ...
        // Save draft
        if (!isEditMode) {
            sessionStorage.setItem('customerFormDraft', JSON.stringify(formData));
            sessionStorage.setItem('customerFormDraft_customCategory', customCategory);
        }
    }, [formData, customCategory, isEditMode]);
    ...
    const handleImageChange = async (e, field) => {
        ...
        try {
            ...
            const { data } = await axios.post('http://localhost:5000/api/customers/upload', formDataToUpload, config);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditMode) {
                await axios.put(`http://localhost:5000/api/customers/${id}`, submitData, config);
                toast.success('Customer updated successfully');
            } else {
                await axios.post('http://localhost:5000/api/customers', submitData, config);
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 pb-12">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold text-gray-800">{isEditMode ? 'Edit Customer' : 'Add New Customer'}</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="glass-card p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input required type="text" name="fullName" value={formData.fullName} onChange={handleChange} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Mobile Number *</label>
                            <input required type="text" name="mobileNumber" value={formData.mobileNumber} onChange={handleChange} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50" />
                        </div>
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-gray-700 mb-1">Customer Photo (Optional)</label>
                            <div className="flex items-center gap-4">
                                {formData.photo && (
                                    <img src={formData.photo} alt="Customer" className="w-16 h-16 rounded-full object-cover border border-gray-200 shadow-sm" />
                                )}
                                <input 
                                    type="file" 
                                    accept="image/*"
                                    ...
                                    className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 disabled:opacity-50" 
                                />
                                {uploadingImage && <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary"></div>}
                            </div>
                        </div>
                    </div>
                </div>

                {/* Product Details Section - Only for New Customer */}
                {!isEditMode && (
                    <>
                        <div className="glass-card p-6">
                ...
                <div className="flex justify-end gap-4">
                    <Link to="/customers" className="px-6 py-3 bg-white border border-gray-200 rounded-xl font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                        Cancel
                    </Link>
